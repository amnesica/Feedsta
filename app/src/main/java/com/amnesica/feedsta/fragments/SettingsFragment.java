...
public class SettingsFragment extends PreferenceFragmentCompat
    ...
    public boolean onPreferenceTreeClick(Preference preference) {
        ...
        if (key.equals(getResources().getString(R.string.export_files)) || key.equals(
                getResources().getString(R.string.import_files))) {
    ...
    private void chooseFolderToExport() {
        ...
        FragmentHelper.showToast(getString(R.string.choose_export_directory), requireActivity(),
                                 requireContext());
    }
    ...
    private void chooseFolderToImport() {
        ...
        FragmentHelper.showToast(getString(R.string.choose_import_directory), requireActivity(),
                                 requireContext());
    }
    ...
    public void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        switch (requestCode) {
            ...
            case 9998:
                ...
                break;
            default:
                FragmentHelper.showToast(getString(R.string.operation_cancelled), requireActivity(),
                                         requireContext());
        }
    }
    ...
    private void exportDataFromApp(final Uri pickedDir) {
        ...
        String rawApplicationName = requireActivity().getApplicationInfo().loadLabel(
                requireActivity().getPackageManager()).toString();
        String applicationName = Character.toLowerCase(rawApplicationName.charAt(0)) +
                                 rawApplicationName.substring(1);
        ...
        if (StorageHelper.checkIfFileExists(StorageHelper.filename_accounts, requireContext())) {
            ...
            if (!successful) {
                FragmentHelper.showToast(getString(R.string.export_failed), requireActivity(),
                                         requireContext());
                return;
            }
        }
        ...
        if (new File(pathToSharedPrefs).exists()) {
            ...
            String inputPathSharedPrefs = "/data".concat("/data/com.amnesica.").concat(applicationName)
                    .concat("/shared_prefs");
            String inputFilename = "com.amnesica." + applicationName + "_preferences.xml";

            successful = StorageHelper.copyFileExport(inputPathSharedPrefs, inputFilename, pickedDir,
                                                      requireContext());
    ...
    private void importDataFromApp(String importPath) {
        ...
        if (new File(accountsPathToImportFrom).exists()) {
            successful = StorageHelper.copyFileImport(new File(accountsPathToImportFrom),
                                                      accountsPathDestination);
            successfulImportAccounts = successful;
            if (!successful) {
                FragmentHelper.showToast(getString(R.string.import_failed), requireActivity(),
                                         requireContext());
                return;
            }
        }
        ...
        if (new File(bookmarksPathToImportFrom).exists()) {
            successful = StorageHelper.copyFileImport(new File(bookmarksPathToImportFrom),
                                                      bookmarksPathDestination);
        ...
        if (new File(sharedPrefsPathToImportFrom).exists()) {
            successful = StorageHelper.copyFileImport(new File(sharedPrefsPathToImportFrom),
                                                      pathToSharedPrefs);
        ...
        if (successfulImportAccounts && successfulImportBookmarks && successfulImportSharedPrefs) {
            FragmentHelper.showToast(getString(R.string.import_successful), requireActivity(),
                                     requireContext());

            // show dialog to restart phone (needed for changes in settings)
            showRestartPhoneDialog();
        } else {
            FragmentHelper.showToast(getString(R.string.some_data_may_not_be_imported), requireActivity(),
                                     requireContext());
        }
    }
    ...
    private void showRestartAppDialog() {
        ...
        alertDialogBuilder = new AlertDialog.Builder(requireContext()).setTitle(
                ...
                .setNegativeButton(R.string.dialog_dark_mode_changed_negative_button,
                                   new DialogInterface.OnClickListener() {
                                       @Override
                                       public void onClick(DialogInterface dialog, int which) {
                                           dialog.dismiss();
    ...
    private void showRestartPhoneDialog() {
        ...
        alertDialogBuilder = new AlertDialog.Builder(requireContext()).setTitle(
                ...
                .setNegativeButton(R.string.dialog_restart_phone_negative_button,
                                   new DialogInterface.OnClickListener() {
                                       @Override
                                       public void onClick(DialogInterface dialog, int which) {
                                           dialog.dismiss();
