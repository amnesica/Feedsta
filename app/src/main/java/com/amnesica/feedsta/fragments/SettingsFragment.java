...
public class SettingsFragment extends PreferenceFragmentCompat implements SharedPreferences.OnSharedPreferenceChangeListener {
    ...
    private void chooseFolderToExport() {
        ...
        startActivityForResult(Intent.createChooser(i, getString(R.string.choose_export_directory)), 9999);
        FragmentHelper.showToast(getString(R.string.choose_export_directory), requireActivity(), requireContext());
    }
    ...
    private void chooseFolderToImport() {
        Intent i = new Intent(Intent.ACTION_OPEN_DOCUMENT_TREE);
        i.addCategory(Intent.CATEGORY_DEFAULT);
        startActivityForResult(Intent.createChooser(i, getString(R.string.choose_import_directory)), 9998);
        FragmentHelper.showToast(getString(R.string.choose_import_directory), requireActivity(), requireContext());
    }
    ...
    public void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        switch (requestCode) {
            ...
            case 9998:
                ...
                break;
            default:
                FragmentHelper.showToast(getString(R.string.operation_cancelled), requireActivity(), requireContext());
        }
    }
     ...
     *
     * @param path String of picked path
     */
    private void showImportConfirmationDialog(final String path) {
        ...
        alertDialogBuilder = new AlertDialog.Builder(requireContext())
                ...
                .setPositiveButton(R.string.dialog_confirm_import_positive_button, new DialogInterface.OnClickListener() {
     ...
     *
     * @param pickedDir picked destination directory
     */
    private void exportDataFromApp(Uri pickedDir) {
        if (pickedDir != null) {
            boolean successful;

            String rawApplicationName = requireActivity().getApplicationInfo()
                    .loadLabel(requireActivity().getPackageManager()).toString();
            String applicationName = Character.toLowerCase(rawApplicationName.charAt(0)) + rawApplicationName.substring(1);
            String pathToSharedPrefs = "/data"
            ...
            if (StorageHelper.checkIfFileExists(StorageHelper.filename_accounts, requireContext())) {
                ...
                if (!successful) {
                    FragmentHelper.showToast(getString(R.string.export_failed), requireActivity(), requireContext());
                    return;
                }
            }
            ...
            if (new File(pathToSharedPrefs).exists()) {
                ...
                String inputPathSharedPrefs = "/data"
                        .concat("/data/com.amnesica.")
                        .concat(applicationName)
                        .concat("/shared_prefs");
                String inputFilename = "com.amnesica." + applicationName + "_preferences.xml";

                successful = StorageHelper.copyFileExport(inputPathSharedPrefs, inputFilename, pickedDir, requireContext());
            ...
            FragmentHelper.showToast(getString(R.string.export_successful), requireActivity(), requireContext());
        } else {
            FragmentHelper.showToast(getString(R.string.export_failed), requireActivity(), requireContext());
        }
    }
     ...
     *
     * @param importPath path of directory
     */
    private void importDataFromApp(String importPath) {
        if (importPath != null) {
            ...
            String accountsPathDestination = requireContext().getFilesDir() + File.separator + StorageHelper.filename_accounts;

            String bookmarksPathToImportFrom = importPath + File.separator + StorageHelper.filename_bookmarks;
            String bookmarksPathDestination = requireContext().getFilesDir() + File.separator + StorageHelper.filename_bookmarks;
            ...
            if (new File(accountsPathToImportFrom).exists()) {
                successful = StorageHelper.copyFileImport(new File(accountsPathToImportFrom), accountsPathDestination, requireContext());
                successfulImportAccounts = successful;
                if (!successful) {
                    FragmentHelper.showToast(getString(R.string.import_failed), requireActivity(), requireContext());
                    return;
                }
            }
            ...
            if (new File(bookmarksPathToImportFrom).exists()) {
                successful = StorageHelper.copyFileImport(new File(bookmarksPathToImportFrom), bookmarksPathDestination, requireContext());
            ...
            if (new File(sharedPrefsPathToImportFrom).exists()) {
                successful = StorageHelper.copyFileImport(new File(sharedPrefsPathToImportFrom), pathToSharedPrefs, requireContext());
            ...
            if (successfulImportAccounts && successfulImportBookmarks && successfulImportSharedPrefs) {
                FragmentHelper.showToast(getString(R.string.import_successful), requireActivity(), requireContext());

                // show dialog to restart phone (needed for changes in settings)
                showRestartPhoneDialog();
            } else {
                FragmentHelper.showToast(getString(R.string.some_data_may_not_be_imported), requireActivity(), requireContext());
            }
        }
    }
    ...
    private void showRestartAppDialog() {
        ...
        alertDialogBuilder = new AlertDialog.Builder(requireContext())
                ...
                .setPositiveButton(R.string.dialog_dark_mode_changed_positive_button, new DialogInterface.OnClickListener() {
                    public void onClick(DialogInterface dialog, int which) {
                        requireActivity().finish();
                    }
                })
                .setNegativeButton(R.string.dialog_dark_mode_changed_negative_button, new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialog, int which) {
                        dialog.dismiss();
                    }
    ...
    /**
     * Show dialog to restart phone (needed for shared preferences to be used by the app)
     */
    private void showRestartPhoneDialog() {
        ...
        alertDialogBuilder = new AlertDialog.Builder(requireContext())
                ...
                .setPositiveButton(R.string.dialog_restart_phone_positive_button, new DialogInterface.OnClickListener() {
                    public void onClick(DialogInterface dialog, int which) {
                        requireActivity().finish();
                    }
                })
                .setNegativeButton(R.string.dialog_restart_phone_negative_button, new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialog, int which) {
                        dialog.dismiss();
                    }
     ...
     * Shows dialog that files are being overwritten if they exist
     *
     * @param pickedDir Uri of picked directory
     */
    private void showExportConfirmationDialog(final Uri pickedDir) {
        ...
        alertDialogBuilder = new AlertDialog.Builder(requireContext())
                ...
                .setPositiveButton(R.string.dialog_confirm_export_positive_button, new DialogInterface.OnClickListener() {
                    public void onClick(DialogInterface dialog, int which) {
                        // do export
