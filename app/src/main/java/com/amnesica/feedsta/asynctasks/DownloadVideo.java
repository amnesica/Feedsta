...
public class DownloadVideo extends AsyncTask<Void, Void, Void> {
    ...
    protected void onPostExecute(Void aVoid) {
        ...
        if (saved) {
            ...
            FragmentHelper.showToast(fragment.getResources().getString(R.string.video_saved),
                                     fragment.requireActivity(), fragment.requireContext());
        } else {
            FragmentHelper.showToast(fragment.getResources().getString(R.string.video_saved_failed),
                                     fragment.requireActivity(), fragment.requireContext());
        }
    }
    ...
    private void showProgressDialog() {
        ...
        if (fragment instanceof PostFragment) {
            ...
            ((PostFragment) fragment).progressDialogBatch.setTitle(
                    fragment.requireContext().getString(R.string.progress_dialog_title_download_video));
            ((PostFragment) fragment).progressDialogBatch.setMessage(fragment.requireContext().getString(
                    R.string.progress_dialog_message_download_selected_posts));
