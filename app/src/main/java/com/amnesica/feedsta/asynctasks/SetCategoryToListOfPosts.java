...
public class SetCategoryToListOfPosts extends AsyncTask<Void, Integer, Void> {
  ...
  public SetCategoryToListOfPosts(
      Fragment fragment,
      List<Post> listOfPostsToEditCategory,
      String category,
  ...
  protected Void doInBackground(Void... voids) {
    if (!isCancelled()
        ...
        && listOfPostsToEditCategory != null) {
    ...
    if (!successful) {
      if (editMode.equals(EditBookmarksType.MOVE_BOOKMARKS)) {
        Toast.makeText(
                ...
                "Moving bookmarks to collection " + category + " failed",
                Toast.LENGTH_SHORT)
            .show();
      }
      if (editMode.equals(EditBookmarksType.RENAME_COLLECTION)) {
        Toast.makeText(
                fragment.requireContext(),
                "Renaming collection to " + category + " failed",
                Toast.LENGTH_SHORT)
            .show();
      }
      if (editMode.equals(EditBookmarksType.REMOVE_FROM_THIS_COLLECTION)) {
        Toast.makeText(
                fragment.requireContext(),
                R.string.removing_bookmarks_from_collection_failed,
                Toast.LENGTH_SHORT)
            .show();
      }
      ...
      if (editMode.equals(EditBookmarksType.MOVE_BOOKMARKS)) {
        Toast.makeText(
                fragment.requireContext(),
                "Successfully moved bookmarks to collection " + category,
                Toast.LENGTH_SHORT)
            .show();
      }
      if (editMode.equals(EditBookmarksType.RENAME_COLLECTION)) {
        Toast.makeText(
                fragment.requireContext(),
                "Successfully renamed collection to " + category,
                Toast.LENGTH_SHORT)
            .show();
      }
      if (editMode.equals(EditBookmarksType.REMOVE_FROM_THIS_COLLECTION)) {
  ...
  private void showProgressDialog() {
    ...
    if (editMode.equals(EditBookmarksType.MOVE_BOOKMARKS)) {
      ...
      progressDialogBatch.setMessage(
          fragment
              .requireContext()
              .getString(R.string.progress_dialog_message_move_selected_posts));
    }
    ...
    if (editMode.equals(EditBookmarksType.REMOVE_FROM_MULTIPLE_COLLECTIONS)) {
      progressDialogBatch.setTitle(
          fragment
              .requireContext()
              .getString(R.string.progress_dialog_title_remove_only_collection));
      progressDialogBatch.setMessage(
          fragment
              .requireContext()
              .getString(R.string.progress_dialog_message_remove_only_collection));
    }
