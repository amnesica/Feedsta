package com.amnesica.feedsta.adapter.collections;
...
public class RecViewAdapterCollections
    extends RecyclerView.Adapter<RecViewAdapterCollections.ViewHolder> {
        ...
        public boolean onActionItemClicked(ActionMode mode, MenuItem item) {
          ...
          if (itemId == R.id.menu_remove_collection_bookmarks) {
            ...
            if (listener != null && !selectedCollections.isEmpty()) {
              ...
              listWithoutAllCollection =
                  listTmpSelectedCollections.stream()
                      .filter(
                          collection ->
                              (collection.getName() != null) && collection.getName().equals("All"))
              ...
              if (listWithoutAllCollection != null && listWithoutAllCollection.size() > 1) {
                if (context != null) {
                  Toast.makeText(
                          context,
                          context.getString(R.string.only_one_collection_can_renamed_at_time),
                          Toast.LENGTH_SHORT)
                      .show();
                }
              }
            ...
            if (listener != null && !selectedCollections.isEmpty()) {
              ...
              if (listTmpSelectedCollections.size() >= 1 && listener != null) {
                listener.resetCollectionInAllSelectedBookmarksOfCategory(
                    listTmpSelectedCollections);
              } else {
                // more than one collection is selected
                if (context != null) {
                  Toast.makeText(
                          context,
                          context.getString(R.string.select_at_least_one_coll),
                          Toast.LENGTH_SHORT)
                      .show();
                }
              }
